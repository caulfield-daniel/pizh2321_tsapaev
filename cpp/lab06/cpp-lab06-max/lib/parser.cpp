...
float Element::ToReal() const {
	CHECK_KIND(REAL);
	return valueReal_;
}
bool Element::ToBoolean() const {
	CHECK_KIND(BOOLEAN);
	return valueBool_;
}
...
const std::vector<Element>& Element::ToList() const {
	CHECK_KIND(LIST);
	return valueList_;
}
const Node& Element::AsNode() const {
	CHECK_KIND(NODE);
	return *nodePtr_;
}
...
Element Element::MakeReal(float v) {
	Element e;
	e.kind_ = Kind::REAL;
	e.valueReal_ = v;
	return e;
}
Element Element::MakeBoolean(bool v) {
	Element e;
	e.kind_ = Kind::BOOLEAN;
	e.valueBool_ = v;
	return e;
}
Element Element::MakeText(const std::string& v) {
	Element e;
	e.kind_ = Kind::TEXT;
	e.valueStr_ = v;
	return e;
}
Element Element::MakeList(const std::vector<Element>& v) {
	Element e;
	e.kind_ = Kind::LIST;
	e.valueList_ = v;
	return e;
}
Element Element::MakeNode(const Node& n) {
	Element e;
	e.kind_ = Kind::NODE;
	e.nodePtr_ = new Node(n);
	return e;
}
		...
		while (std::getline(in, line)) {
			...
			if (clean.front() == '[' && clean.back() == ']') {
				...
				while (pos < path.size()) {
					...
					std::string part = path.substr(pos, dot == std::string::npos
															? std::string::npos
															: dot - pos);
...
Element interpret(const std::string& s) {
	...
	// Boolean
	if (s == "true" || s == "false") return Element::MakeBoolean(s == "true");
	// Array
	if (s.front() == '[' && s.back() == ']') {
