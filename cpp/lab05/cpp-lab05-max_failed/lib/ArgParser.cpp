...
void ArgParser::ParseWithEqualSign(const std::vector<std::string_view>& args,
								   size_t cur_arg_pos, size_t equal_sign_pos) {
...
void ArgParser::ParseWithoutEqualSign(const std::vector<std::string_view>& args,
									  size_t& cur_arg_pos) {
	...
	if (count_dashes == 1 || count_dashes == 2) {
		...
		// No equal sign but 1 or 2 dashes and not a flag = args with type --arg
		// value
		for (Argument* added_arg : arguments_) {
			if (!IsEquals(added_arg, arg_name) ||
				added_arg->type == ArgumentType::Bool) {
				continue;
			}
		...
		for (const char& flag : arg_name) {
			for (auto& added_args : arguments_) {
				if (added_args->type != ArgumentType::Bool ||
					(added_args->arg != arg_name &&
					 added_args->short_arg != flag)) {
					continue;
				}
		...
		switch (arg->type) {
			case ArgumentType::Int:
				if (arg->is_positional == false && arg->int_value == -1) {
					return false;
				} else if (arg->min_args_count != INT_MAX &&
						   arg->multi_store_int->size() < arg->min_args_count) {
					return false;
				}

				break;
			case ArgumentType::String:
				if (arg->multi_store_str && arg->min_args_count != INT_MAX &&
					arg->multi_store_str->size() < arg->min_args_count) {
					return false;
				} else if (arg->is_positional == false &&
						   arg->str_value.empty()) {
					return false;
				}
...
Argument& ArgParser::AddHelp(char short_arg, const std::string& arg,
							 const std::string& desc) {
	Help_ = new Argument(short_arg, arg, desc);
	return *Help_;
}

bool ArgParser::Help() const { return is_help_enabled_; }
...
std::string ArgParser::HelpDescription() const {
	...
	for (Argument* arg : arguments_) {
		...
		if (arg->min_args_count != INT_MAX) {
			desc +=
				"[repeated, min args = " + std::to_string(arg->min_args_count) +
				"] ";
		}
...
bool ArgParser::IsEquals(const Argument* arg,
						 const std::string_view& arg_name) const {
	return arg && (arg->arg == arg_name ||
				   (arg_name.size() == 1 && arg->short_arg == arg_name[0]));
}
