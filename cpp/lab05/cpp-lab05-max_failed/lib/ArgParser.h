...
class ArgParser {
   public:
	...
	void ParseWithEqualSign(const std::vector<std::string_view>& args,
							size_t cur_arg_pos, size_t equal_sign_pos);
	void ParseWithoutEqualSign(const std::vector<std::string_view>& args,
							   size_t& cur_arg_pos);
	...
	Argument& AddFlag(char short_arg, const std::string& arg,
					  const std::string& desc);
	Argument& AddHelp(char short_arg, const std::string& arg,
					  const std::string& desc);
